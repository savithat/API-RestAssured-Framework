pipeline 
{
    ...
    steps {
        ...
        def exitCode = sh(script: "docker run --name api-restassured-fw${BUILD_NUMBER} -e MAVEN_OPTS='-Dsurefire.suiteXmlFiles=src/test/resources/testrunners/testng_regression.xml' savithatm/api-restassured-fw-pipe:latest", returnStatus: true)
            if (exitCode != 0) {
